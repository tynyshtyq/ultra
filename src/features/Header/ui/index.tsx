...
const Header: FC<Props> = ({user}) => {
    ...
    const handleStudyBuddies = React.useCallback(() => {
        router.push(ROUTES.STUDYBUDDY.get());
        setLoading(true)
    }, [])
    ...
    return (
        <nav className='w-full p-4 flex items-center justify-between border-b-m border-vista border-opacity-20 shadow-lg'>
            ...
            <div className='flex w-max items-center gap-2'>
                ...
                <Button.Secondary onClick={handleSync} className='phone:hidden'>Sync with registrar</Button.Secondary>
                <Button.Secondary onClick={handleSync} className='desktop:hidden'>Sync</Button.Secondary>
